					 ...
					 nccl_ofi_topo_data_iterator_t *data_iter)
{
	...
	while (node) {
		if (!node->userdata) {
			...
			user_data->relevant_numa_node = NULL;
			nccl_ofi_inc_user_data_iter(data_iter);
		}
		node = node->parent;
	}
...
/*
 * @brief    Traverse topology tree in depth-first search to mark nodes along the path
 *        between a NUMA node and a Package with their relevant NUMA node.
 *        This function identifies nodes that lie on the path from a Package to a NUMA node
 *        and associates them with the nearest NUMA node in their subtree.
 *
 * @param    topo
 *        hwloc topology handle
 * @param    node
 *        Current node being visited in the DFS traversal
 * @param    relevant_numa_node
 ...
 */
hwloc_obj_t dfs_mark_relevant_numa_node(hwloc_topology_t topo, hwloc_obj_t node, hwloc_obj_t relevant_numa_node, bool found_numa, bool found_package) {
	hwloc_obj_t child = NULL;
	hwloc_obj_t latest_numa_node = relevant_numa_node;
	...
	/* Base case 1. Found both NUMA and Package. We can return at this point. */
	if (found_package && found_numa) {
		...
		((nccl_ofi_topo_data_t *)(node->userdata))->relevant_numa_node = latest_numa_node;

		/* For HWLOC 1.x we expect the bottom most node to be Package.
		 * For HWLOC 2.x we expect the bottom most node to be NUMA Node.
		 * Only when the return value is `True`, we want to set the relevant NUMA Node ID. */
		return latest_numa_node;
	}

	while ((child = hwloc_get_next_child(topo, node, child))) {
		hwloc_obj_t ret = dfs_mark_relevant_numa_node(topo, child, latest_numa_node, found_numa, found_package);
		/* The return pointer, if not NULL, indicates that somepoint down the tree, both NUMANODE and Package were found.
		 * And the return pointer is set to the relevant NUMA Node's address. */
		bool set_userdata = (found_package || found_numa) && (ret);
		if (set_userdata) {
			/* The current node is long the NUMA Node <--> Package path. */
			if (!(node->userdata)) {
				nccl_ofi_topo_data_t *topo_data = (nccl_ofi_topo_data_t *)malloc(sizeof(nccl_ofi_topo_data_t));
				topo_data->is_along_nic_or_gpu_path = false;
				node->userdata = topo_data;
			}
			((nccl_ofi_topo_data_t *)(node->userdata))->relevant_numa_node = ret;
			return ret;
		}
	}
				  ...
				  struct fi_info *info_list)
{
	...
	dfs_mark_relevant_numa_node(ofi_topo->topo, obj, NULL, false, false);
#endif
...
static int write_nccl_topo_rec(hwloc_topology_t topo, hwloc_obj_t node, FILE *file, int indent, int bridge_depth)
{
	...
	/* HWLOC 2.x: For PACKAGE nodes, only call cpu_opening_tag if relevant_numa_id is set */
		...
		if (topo_data->relevant_numa_node) {
			if ((ret = write_cpu_opening_tag(topo_data->relevant_numa_node, file, indent))) {
				return ret;
			}
			close_numanode = true;
			indent += indent_offset;
